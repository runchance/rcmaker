...
- Treat `vendor/` as read-only for normal application work. It may be edited only when the developer explicitly identifies the current task as framework maintenance and explicitly authorizes the target installed package. Keep that exception limited to the named package and task, and deliver tests plus documentation with framework changes.
