...
10. **自定义能力必须遵守项目目录边界。** 只有确认 rcmaker 没有对应能力后，才能新增实现；新增文件必须按职责放入现有 `apps/`、`support/`、`config/`、`public/`、`view/`、`tests/`、`scripts/` 或文档目录。禁止为了省事在项目根目录创建控制器、服务、模型、任务、脚本、测试、样例或临时文件。
...
- 不得在项目根目录随意创建业务 PHP、服务、模型、任务、测试、演示、下载文件或临时输出；必须移动到职责目录，或在任务结束前删除。
