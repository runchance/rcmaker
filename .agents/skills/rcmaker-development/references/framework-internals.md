...
排查生命周期问题、框架缺陷或做性能优化时必须阅读本文件，并以当前安装源码为行为依据。普通项目任务只能读取 `vendor/runchance/rcmaker-framework`；开发人员明确声明当前任务属于框架维护并授权该包时，框架开发者可以直接修改，同时必须补齐测试和文档。不要把这里的实现细节当成永远不变的公共 API。
...
以下要求用于分析和维护框架源码。未获得明确框架维护授权时，当前项目的 `vendor/` 仍然只读；获得授权后也只能修改指定框架包，不能改动无关依赖。
