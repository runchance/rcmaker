...
排查生命周期问题、框架缺陷或做性能优化时必须阅读本文件，并以当前安装源码为行为依据。`vendor/runchance/rcmaker-framework` 只允许读取和检索，任何时候都不得直接修改；需要修复框架时，应在 rcmaker-framework 的可维护源仓库中完成并通过 Composer 发布、更新。不要把这里的实现细节当成永远不变的公共 API。
...
以下要求用于分析当前安装源码，并指导在框架的可维护源仓库中实施修复。当前项目的 `vendor/` 始终只读，不允许直接写入补丁或临时改动。
