...
开发人员可以在当前 AI 交互中明确指定非标准路径或批准目录例外。此时按指定范围执行并在交付说明中记录用途；一次特许不自动延续到其他文件、后续任务或框架复用等其他规则。没有明确授权时仍按上表放置。

## 人工审查友好的代码

AI 生成的代码首先要让开发人员能读懂、能维护、能审查：

- 每个新增或修改的类使用 PHPDoc 说明职责、所属层次和典型用法。
- 每个新增或修改的方法说明用途与调用方式；公开方法同时说明参数、返回值、异常和必要示例，私有方法说明在内部流程中的作用。
- 保留正常缩进、换行、空白和清晰命名，不输出压缩式 PHP，不把多个业务步骤塞在一行。
- 复杂逻辑拆成有业务名称的小方法；注释重点解释原因、约束、状态和边界，不逐句翻译代码。
- 文档、PHPDoc、测试示例与实现同步更新，示例遵守 rcmaker 组件复用规则。

审查新增代码时，应能从类注释理解用途，从公开方法注释知道如何调用，再顺着清晰的方法拆分理解实现，而不需要先运行代码或猜测变量含义。
