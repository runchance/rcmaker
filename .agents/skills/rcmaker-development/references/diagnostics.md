...
PHP 新版本出现第三方库弃用提示，例如旧 Workerman `case ...;` 语法，应判断为 PHP/依赖兼容问题。优先升级到兼容版本或在依赖上游修复，不通过关闭全部错误报告掩盖真实问题。普通项目排障不得修改 `vendor/`；只有开发人员明确授权的框架维护任务可以修改指定包，并且必须形成可测试、可发布的修复。
...
- 依赖问题在上游源码、项目依赖声明或可维护补丁源中解决，再通过 Composer 重新安装；整个处理过程不直接修改项目 `vendor/`。
- 性能结论包含错误数和可复现命令。
